...
function handler:OnKeyFrame(keyframeName)
	...
	elseif keyframeName == "Complete" then
		...
		if self.loadedAnimations.action_open then
			self.loadedAnimations.action_open:Stop()
		end
...
function handler:equip(wepName)
	...
	if self.viewmodel.attachments:FindFirstChild("Aim") then
		self.viewmodel.offsets.aim.Value = self.viewmodel.attachments.Aim.CFrame:ToObjectSpace(
			self.viewmodel.HumanoidRootPart.CFrame
		)
	end
	...
	self.loadedAnimations.reload2 = self.settings.animations.viewmodel.reload2 ~= nil
			and self.viewmodel.AnimationController:LoadAnimation(
				self.settings.animations.viewmodel.reload2
			)
		or nil
	self.loadedAnimations.equip = self.settings.animations.viewmodel.equip ~= nil
			and self.viewmodel.AnimationController:LoadAnimation(
				self.settings.animations.viewmodel.equip
			)
		or nil
	self.loadedAnimations.action_open = self.settings.animations.viewmodel.action_open ~= nil
			and self.viewmodel.AnimationController:LoadAnimation(
				self.settings.animations.viewmodel.action_open
			)
		or nil
	self.loadedAnimations.sprint = self.viewmodel.AnimationController:LoadAnimation(
		self.settings.animations.viewmodel.sprint
	)

	--[[
		if the gun has certain animations; connect them to the keyframereached handler, else move on
	]]
	self.loadedAnimations.reload.KeyframeReached:Connect(function(keyframeName)
		self:OnKeyFrame(keyframeName)
	end)
	if self.loadedAnimations.reload2 then
		self.loadedAnimations.reload2.KeyframeReached:Connect(function(keyframeName)
			self:OnKeyFrame(keyframeName)
		end)
	end
	if self.loadedAnimations.action_open and self.weaponData[wepName].mag <= 0 then
		self.loadedAnimations.action_open:Play(0)
	end
...
function handler:remove(override)
	if not override then
		if self.sprinting or self.reloading or self.firing then
			return false
		end
		self:aim(false)
		...
		self.equipped = false
		if self.loadedAnimations.equip then
			self.loadedAnimations.equip:Play(0.1, 1, -2)
		end --play equip anim in reverse
...
function handler:aim(toaim)
	if self.sprinting or self.disabled or not self.equipped or self.reloading then
		return
	end
	self.aiming = toaim
	...
	if toaim then
		...
		local tweeningInformation = TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
		...
		tweenService
			:Create(
				...
				tweeningInformation,
				{
					FieldOfView = self.viewmodel.offsets:FindFirstChild("aimFov")
							and self.viewmodel.offsets.aimFov.value
						or 70,
				}
			)
			:Play()
...
function handler:reload()
	if self.sprinting or self.aiming or self.reloading or self.disabled or not self.equipped or self.firing then
		return
	end
	-- no ammo? then no reload
	if self.weaponData[self.curWeapon].spare <= 0 then
		return
	end
	self.reloading = true
...
function handler:sprint(tosprint)
	if self.reloading or self.disabled or not self.equipped or self.firing then
		return
	end
...
function handler:clientReloadCalc()
	...
	if
		(
			self.weaponData[self.curWeapon].spare
			- (self.weaponData[self.curWeapon].settings.firing.magCapacity - self.weaponData[self.curWeapon].mag)
		) <= 0
	then
		self.weaponData[self.curWeapon].mag = self.weaponData[self.curWeapon].mag
			+ self.weaponData[self.curWeapon].spare
		self.weaponData[self.curWeapon].spare = 0
		-- if the spare ammo has more than the mags worth
	else
		self.weaponData[self.curWeapon].spare = self.weaponData[self.curWeapon].spare
			- (self.weaponData[self.curWeapon].settings.firing.magCapacity - self.weaponData[self.curWeapon].mag)
...
function handler:fireRecoil()
	...
	local rec = Vector3.new(
		Random:NextNumber(patt.min.x, patt.max.x),
		Random:NextNumber(patt.min.y, patt.max.y),
		Random:NextNumber(patt.min.z, patt.max.z)
	) * (self.aiming and 0.5 or 1)
...
function handler:update(deltaTime)
	if self.viewmodel then
		...
		if self.aiming then
			mouseDelta = mouseDelta * aimingSwaySpeed
		else
			mouseDelta = mouseDelta * swaySpeed
		end
		...
		if self.sprinting then
			movementSway = Vector3.new(getBobbing(7, 2, 0.04), getBobbing(7, 2, 0.04), getBobbing(33, 2, 0.52))
			self.viewmodel.HumanoidRootPart.CFrame = self.viewmodel.HumanoidRootPart.CFrame:ToWorldSpace(
				CFrame.new(
					walkCycle.x / 2
						+ ((0.2 * math.sin(5 * 1 * tick())) * math.clamp(math.min(velocity.Magnitude, 32), 0, 1)),
					walkCycle.y / 2,
					0
				)
			)
			...
			movementSway = Vector3.new(
				getBobbing(10, speed, modifier),
				getBobbing(5, speed, modifier),
				getBobbing(5, speed, modifier)
			)
			self.viewmodel.HumanoidRootPart.CFrame = self.viewmodel.HumanoidRootPart.CFrame:ToWorldSpace(
				CFrame.new(walkCycle.x / 2, walkCycle.y / 2, 0)
			)
		end
		-- Rotate rootpart based on sway
		self.viewmodel.HumanoidRootPart.CFrame = self.viewmodel.HumanoidRootPart.CFrame
			* CFrame.Angles(-sway.y, -sway.x, 0)
		self.viewmodel.HumanoidRootPart.CFrame = self.viewmodel.HumanoidRootPart.CFrame
			* CFrame.Angles(-walkCycle.x, walkCycle.y, 0)
...
function handler:fire(tofire)
	if self.sprinting or self.reloading or self.disabled or not self.equipped or self.firing and tofire then
		return
	end
	self.firing = tofire
	if not tofire or self.weaponData[self.curWeapon].mag <= 0 then
		return
	end -- no ammo
	if (tick() - lastClick) < (60 / self.settings.firing.rpm) then
		return
	end -- clicking faster that fire rate
	if not self.viewmodel:FindFirstChild("receiver") then
		return
	end -- no gun
	...
	while self.firing and clickTick == lastClick do -- break if the player clicked again to prevent overlapping firing loops
		...
		for i = 1, self.settings.firing.shots or 1 do -- prime data for each shot based on shots per trigger fire in the settings
			...
			local spread = Vector3.new(
				(math.random(-baseSpread, baseSpread) / 250) * distance,
				(math.random(-baseSpread, baseSpread) / 250) * distance,
				(math.random(-baseSpread, baseSpread) / 250) * distance
			)
			...
			if self.settings.firing.shotgun then
				...
				-- fire each round
				if self.weaponData[self.curWeapon].mag <= 0 then
					continue
				end
				self.loadedAnimations.fire:Play(0, 1, 2)

				-- this is a handler for fastcast, it will handle bullet firing
				fastcastHandler:fire(
					origin,
					direction,
					self.settings,
					false,
			...
			if self.weaponData[self.curWeapon].mag <= 0 and not self.settings.firing.shotgun then
				break
			end -- if its a shotgun then continue, else end because no ammo (each pellet wont take 1 ammo)
			if self.settings.firing.burstDelay then
				task.wait(self.settings.firing.burstDelay)
			end -- burst delay for burst fire weapons
		end

		-- send all shotgun pellets to be handled at once
		if self.settings.firing.shotgun then
			fastcastHandler:fireShotgun(
				pellets,
				self.settings,
				false,
				game.Players.LocalPlayer.Character,
		...
		if
			not self.settings.firing.auto
			or not UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
			or clickTick ~= lastClick
		then
			break
		end
		if not self.curWeapon or self.weaponData[self.curWeapon].mag <= 0 then
			break
		end -- break if no ammo
	end
	self.haltFire = false
	-- last round bolt hold open anim if applicable
	if self.loadedAnimations.action_open and self.weaponData[self.curWeapon].mag <= 0 then
		self.loadedAnimations.action_open:Play(0)
	end
end
